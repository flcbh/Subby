...
namespace Subby.Controllers
{
    ...
    public class AdvertsController : Controller
    {
            ...
            int miles, int page)
        {
            ...
            {
                Items = adverts.ApplyPagination(new PagingOptions(), new LastContent.Utilities.Models.SearchOptions<Advert>()),
        ...
        {
            var currentUser = _repository.Linq<User>().FirstOrDefault(x => x.Email == User.Identity.Name);
            _cacheKeys.ForEach(x =>
            {
                _cache.Clear(x);
            });
            var query = _repository.Linq<Advert>().Where(x => x.User == currentUser).ToPagination(page, 20);
            
            PagedResult<Advert> result = new PagedResult<Advert>();
